...
const scopeChoices = (config) => {
  ...
  config.scopes.forEach((scope) => {
    const transformScope = config.lowercaseScopes ? scope.value.toLowerCase() : scope.value;
    const description = scope.description || '';

    choicesList.push({
      value: transformScope,
      name: `${chalk.bold(transformScope)} ${description}`,
    });
  });
...
const questions = (config) => {
  ...
  const scopeChoicesList = scopeChoices(config);
  const questionsList = [
    {
      type: 'list',
      name: 'type',
